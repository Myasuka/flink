...
public class RocksDBAccessMetric implements AutoCloseable {
	...
	RocksDBAccessMetric(MetricGroup metricGroup, int sampleCountInterval, long histogramWindowSize) {
		...
		this.histogramSupplier = () -> new com.codahale.metrics.Histogram(new SlidingTimeWindowReservoir(histogramWindowSize, TimeUnit.SECONDS));
	}

	boolean checkAndUpdateGetCounter(final int columnFamilyHandleId) {
		return metricsSampleEnabled && countersPerColumnFamily.get(columnFamilyHandleId).checkAndUpdateGetCounter();
	}

	boolean checkAndUpdatePutCounter(final int columnFamilyHandleId) {
		return metricsSampleEnabled && countersPerColumnFamily.get(columnFamilyHandleId).checkAndUpdatePutCounter();
	}

	boolean checkAndUpdateDeleteCounter(final int columnFamilyHandleId) {
		return metricsSampleEnabled && countersPerColumnFamily.get(columnFamilyHandleId).checkAndUpdateDeleteCounter();
	}

	boolean checkAndUpdateMergeCounter(final int columnFamilyHandleId) {
		return metricsSampleEnabled && countersPerColumnFamily.get(columnFamilyHandleId).checkAndUpdateMergeCounter();
	}

	boolean checkAndUpdateSeekCounter(final int columnFamilyHandleId) {
		return metricsSampleEnabled && countersPerColumnFamily.get(columnFamilyHandleId).checkAndUpdateSeekCounter();
	}

	boolean checkAndUpdateNextCounter(final int columnFamilyHandleId) {
		return metricsSampleEnabled && countersPerColumnFamily.get(columnFamilyHandleId).checkAndUpdateNextCounter();
	}

	public void updateHistogram(final int columnFamilyHandleId, final String metricName, final long durationNanoTime) {
		this.histogramMetrics.get(columnFamilyHandleId).computeIfAbsent(metricName, (k) -> {
			HistogramWrapper histogram = new HistogramWrapper(histogramSupplier.get());
			columnFamilyMetricGroups.get(columnFamilyHandleId).histogram(metricName, histogram);
			return histogram;
	...
	void registerColumnFamily(String columnFamilyName, ColumnFamilyHandle handle) {
		...
		columnFamilyMetricGroups.putIfAbsent(columnFamilyId, metricGroup.addGroup(COLUMN_FAMILY_KEY, columnFamilyName));
	...
	public static class Builder implements Serializable {
		...
		private long histogramSlidingWindow = RocksDBOptions.LATENCY_TRACK_SLIDING_WINDOW.defaultValue();
		...
		public RocksDBAccessMetric build() {
			if (isEnabled) {
				return new RocksDBAccessMetric(Preconditions.checkNotNull(metricGroup), sampleInterval, histogramSlidingWindow);
			} else {
				return null;
			}
		}
