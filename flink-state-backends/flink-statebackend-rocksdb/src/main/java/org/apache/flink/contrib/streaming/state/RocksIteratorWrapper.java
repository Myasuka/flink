...
public class RocksIteratorWrapper implements RocksIteratorInterface, Closeable {
	...
	public RocksIteratorWrapper(@Nonnull RocksIterator iterator, @Nullable RocksDBAccessMetric accessMetric, @Nullable ColumnFamilyHandleWrapper columnFamilyHandle) {
		...
		Preconditions.checkArgument(!trackLatencyEnabled || columnFamilyHandle != null,
			"Should provide column family handle when enable latency track.");
	}
	...
	private void seekAndUpdateMetric(final byte[] target) {
		if (accessMetric.checkAndUpdateSeekCounter(columnFamilyHandleWrapper.getColumnFamilyId())) {
			...
			accessMetric.updateHistogram(columnFamilyHandleWrapper.getColumnFamilyId(), RocksDBAccessMetric.SEEK_LATENCY, end - start);
		} else {
			originalSeek(target);
		}
	}
	...
	private void nextAndUpdateMetric() {
		if (accessMetric.checkAndUpdateNextCounter(columnFamilyHandleWrapper.getColumnFamilyId())) {
			...
			originalNext();
			long end = System.nanoTime();
			accessMetric.updateHistogram(columnFamilyHandleWrapper.getColumnFamilyId(), RocksDBAccessMetric.NEXT_LATENCY, end - start);
		} else {
			originalNext();
		}
	}
