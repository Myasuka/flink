...
public class RocksDBOptions {
	...
	public static final ConfigOption<Integer> CHECKPOINT_TRANSFER_THREAD_NUM = ConfigOptions
		...
		.defaultValue(1)
		.withDescription(String.format("The number of threads (per stateful operator) used to transfer (download and upload) files in RocksDBStateBackend. " +
			"If %s is enabled, this option would be fixed as 1.", CheckpointingOptions.CHECKPOINT_CONCAT_STREAM_ENABLED));
	...
	@Documentation.Section(Documentation.Sections.STATE_BACKEND_ROCKSDB)
	public static final ConfigOption<Boolean> LATENCY_TRACK_ENABLED = ConfigOptions
		.key("state.backend.rocksdb.latency-track-enabled")
		.booleanType()
		.defaultValue(false)
		.withDescription("Whether to track latency of RocksDB actions, e.g put/get/delete/seek/next.");

	@Documentation.Section(Documentation.Sections.STATE_BACKEND_ROCKSDB)
	public static final ConfigOption<Integer> LATENCY_TRACK_SAMPLE_INTERVAL = ConfigOptions
		.key("state.backend.rocksdb.latency-track-sample-interval")
		.intType()
		.defaultValue(100)
		.withDescription(String.format("The sample interval of latency track once '%s' is enabled. " +
			"The default value is 100, which means we would track the latency every 100 access requests.", LATENCY_TRACK_ENABLED.key()));

	@Documentation.Section(Documentation.Sections.STATE_BACKEND_ROCKSDB)
	public static final ConfigOption<Long> LATENCY_TRACK_SLIDING_WINDOW = ConfigOptions
		.key("state.backend.rocksdb.latency-track-sliding-window")
		.longType()
		.defaultValue(Time.seconds(10).toMilliseconds())
		.withDescription(String.format("The sliding window of histogram to record the latency once '%s' is enabled. " +
			"The default value is 10 seconds, which means we stores only the measurements made in the last 10 seconds.", LATENCY_TRACK_ENABLED.key()));
}
